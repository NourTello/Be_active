...
export function MealPlanForm({ onSubmit, isLoading }: MealPlanFormProps) {
  ...
  const { register, handleSubmit, formState: { errors }, setValue } = useForm<FormData>({
    ...
    defaultValues: {
      ...
      targetWeight: (profile.targetWeight || profile.weight || undefined) as number | undefined,
  ...
  // Sync target weight whenever BMI calculation updates it
  useEffect(() => {
    if (profile.targetWeight) {
      setValue('targetWeight', profile.targetWeight as number, { shouldValidate: false });
    }
  }, [profile.targetWeight, setValue]);

  // Also sync current weight and height from BMI form
  useEffect(() => {
    if (profile.weight) setValue('currentWeight', profile.weight as number, { shouldValidate: false });
    if (profile.height) setValue('height', profile.height as number, { shouldValidate: false });
    if (profile.age) setValue('age', profile.age as number, { shouldValidate: false });
    if (profile.gender) setValue('gender', profile.gender as any, { shouldValidate: false });
  }, [profile.weight, profile.height, profile.age, profile.gender, setValue]);

  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      className="bg-card rounded-3xl p-6 md:p-8 shadow-xl shadow-accent/5 border border-border"
    >
      <div className="flex items-center gap-3 mb-6 pb-6 border-b border-border/50">
        ...
        <div>
          <h2 className="text-2xl font-display font-bold text-foreground">{t.nutritionPlan}</h2>
          <p className="text-muted-foreground text-sm mt-1">{t.nutritionPlanSub}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="space-y-2">
          ...
          <input
            {...register('sport')}
            className="w-full px-4 py-3 rounded-xl bg-muted/50 border border-border focus:border-accent focus:ring-4 focus:ring-accent/10 transition-all outline-none text-foreground"
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-sm font-semibold text-foreground">{t.currentWt}</label>
            <input
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold text-foreground flex items-center gap-1">
              <Target className="w-4 h-4 text-accent" /> {t.targetWt}
              {profile.targetWeight ? (
                <span className="ml-1 flex items-center gap-0.5 px-2 py-0.5 bg-primary/10 text-primary text-xs rounded-full font-medium">
                  <Sparkles className="w-3 h-3" /> {t.fromBmi}
                </span>
              ) : null}
            </label>
            <input
